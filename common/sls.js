...
class Client {
  ...
  getLogs({
    ...
    const __query__ = Object.assign({}, data, {
      type: 'log',
    ...
    if(query != '' && query != null) {
      __query__.query = query
    }
    const path = `/logstores/${logstoreName}`;
    return this._request('GET', projectName, path, __query__, null, {}, options);
  }
...
if(process.env.NODE_ENV == 'prod' || process.env.NODE_ENV == 'test') {
  region = 'cn-shanghai-intranet'
}

module.exports = new Client({
    accessKeyId: config.aliyun.log.AccessKeyId,
    accessKeySecret: config.aliyun.log.AccessKeySecret,
    region: region
});
